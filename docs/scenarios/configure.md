---
generated_at: 2026-09-24T14:05:00+00:00
...
`Camera::configure()` 호출은 `Private::isAccessAllowed()` 검증과 `StreamConfiguration::setStream()` 설정을 거쳐 `IPU3CameraConfiguration::validate()`로 진입하며, 이 과정에서 `CameraSensorLegacy`와 `CameraSensorRaw`가 순차적으로 `computeTransform()`를 수행합니다. 각 센서 클래스는 `transform::operator/()`, `*()`, `-()`, `!()`, `&()` 연산자를 호출하여 회전 변환을 계산하지만, 구체적인 실행 순서는 메서드 목록만으로는 판단할 수 없으므로 `src/libcamera/sensor/camera_sensor_legacy.cpp:961`와 `src/libcamera/sensor/camera_sensor_raw.cpp:1073`의 실제 호출 경로를 확인해야 합니다.

`IPU3CameraConfiguration`는 `CIO2Device::sensor()`를 통해 센서 정보를 가져오며, `Size::Size()` 생성자를 통해 크기 객체를 초기화합니다. `Size::Size()` 정의는 `include/libcamera/geometry.h:54`에 위치하지만, 이 호출이 스레드 안전성을 보장하는지 또는 콜백 전달 시점에는 명확한 근거가 부족하므로 해당 동작의 세부 사항을 확인해야 합니다.
